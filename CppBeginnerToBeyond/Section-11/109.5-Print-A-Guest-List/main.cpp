 ...
 * Begin by completing the function prototypes print guest_list and clear guest list which are both passed three string reference
 * variables . Ensure that the variables cannot be modified within the print guest list function by using the keyword const in the parameter list.
 * Now that the function prototypes have been declared, at the bottom of the file define both functions.
 * Both the print _guest_list and clear _guest_list functions will be passed the string arguments guest 1, guest 2, and guest 3 which have all been provided to you.
 * The print guest_list function should print the guest list in the order of guest_1, guest_2, guest_3, with each entry being printed on a new line. For testing purposes, 
 * please ensure that a newline is printed after guest_3 either through the use of endl; or the newline or character "\n"
 * The clear guest list function should replace the string values contained within guest_1, guest_2, and guest_3 with the string value a single space.
